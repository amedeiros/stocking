...
def screener(message, filters):
    ...
    message.send_webapi('', json.dumps(
        utils.attachments(message, title=f"FinViz Screener Results", title_link=utils.html_url(filename))))
...
def ticker_performance(message, ticker):
    ...
    message.send_webapi('', json.dumps(utils.attachments(
        message, title=f"Performance for {ticker}", title_link=utils.html_url(filename))))
...
def ticker_technical(message, ticker):
    ...
    utils.store_html(df.to_html(classes="table table-striped"), filename)
    message.send_webapi('', json.dumps(utils.attachments(
        message, title=f"Technical for {ticker}", title_link=utils.html_url(filename))))
...
def ticker_ownership(message, ticker):
    ...
    utils.store_html(df.to_html(classes="table table-striped"), filename)
    message.send_webapi('', json.dumps(utils.attachments(
        message, title=f"Ownership for {ticker}", title_link=utils.html_url(filename))))
...
def ticker_financial(message, ticker):
    ...
    utils.store_html(df.to_html(classes='table table-striped'),
                     f"{ticker}_financial.html")

    message.send_webapi('', json.dumps(utils.attachments(
        message, title=f"Financials for {ticker}", title_link=utils.html_url(f"{ticker}_financial.html"))))
