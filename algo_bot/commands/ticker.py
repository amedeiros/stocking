...
def candlestick(message, ticker, interval="60min"):
    ...
    message.send_webapi('', json.dumps(utils.attachments(
        message, title=f"Candlestick Chart For {ticker}", title_link=utils.html_url(filename))))
...
def ticker(message, ticker):
    ...
    message.send_webapi('', json.dumps(
        utils.attachments(message, text=df["Description"][0], title=f"Overview for {ticker}", title_link=utils.html_url(filename))))
